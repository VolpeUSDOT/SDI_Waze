# Tennesee crash data initial formatting and exploration
# Jessie prepared this one for Jonathan to start with some simple data analysis.

# Setup ----

# Check for package installations
getwd() # this is the default working directory, usually will be the "C:/Users/[your user name]/Documents"
codeloc <- "~/GitHub/SDI_Waze" # you'll see the R code location once you cloned the GitHub repository
dataloc <- "//vntscex.local/DFS/Projects/PROJ-OS62A1/SDI Waze Phase 2/Data/TN/Crash"
library(tidyverse) 
library(lubridate)
library(dplyr)
library(ggplot2)
...
setwd("~/workingdata/TN") # you can create your own working folder in any of your local directory

# Data explore

# Crash ----
# look at Crash/vwCollision.txt
# First time need to read from txt, afterwards can read from RData, ~ 2 seconds vs 5 minutes. Every column is read as character. Create the crash data as an R data file or load the already exist R file. 
if(length(grep("TN_Crash.RData", dir('Crash')))==0){
  crash <- read.csv(paste0(dataloc,"/vwCollision.txt"), sep = '|')
