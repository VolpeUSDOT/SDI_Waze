# Updated for Tennessee, only Waze data in this case (independent of TN Crash data)

# Aggregation of Waze varaibles by grid cell
# Goal: create a gridded data set where grid cell contain the count of Waze variables
# Follows MakeSpaceTimeGrids_TN_WazeOnly.R
...
for(g in grids){ # g = grids[1]
  ...
  tlfiles <- dir(temp.outputdir)[grep("WazeOnlyTimeHexAll_", dir(temp.outputdir))]
  ...
  foreach(j = todo.months, .packages = c("dplyr", "lubridate", "utils")) %dopar% {
    ...
    load(file.path(wazemonthdir, paste0("grid.overlay.waze.tn.", g,"_", j, ".RData"))) # includes both waze (link.waze.tn) and TN crash (crash.df) data, with grid for central and neighboring cells
    ...
    load(file.path(temp.outputdir, paste0("WazeOnlyHexTimeList_", j, "_", g, "TN.RData")))
    ...
    # names(Waze.hex.time)
    names(Waze.hex.time)[names(Waze.hex.time) %in% 'alert_uuid'] = 'uuid.waze'
    ...
    fn = paste("WazeOnlyTimeHexAll_", j,"_", g, ".RData", sep="")
