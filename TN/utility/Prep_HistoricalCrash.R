...
if(length(grep(prepname, dir(file.path(localdir, "Crash")))) == 0) { # if doen't exist in TN/Crash, make it
  
  cat("Preparing", "TN_Crash", g, "\n")
  
  load(file.path(localdir, 'Crash', 'TN_Crash_Simple_2008-2018.RData'))
  ...
  system(paste("aws s3 cp",
               file.path(localdir, "Crash", paste0(prepname, ".RData")),
               file.path(teambucket, state, "Crash", paste0(prepname, ".RData"))))

} else {
  load(file.path(localdir, "Crash", paste0(prepname, ".RData")))
}
