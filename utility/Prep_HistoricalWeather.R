...
# Based on TN work, here runnign for CT, MD, UT, and VA for all of 2018 for Phase I journal article
...
for(state in states){ # state = "CT"
  ...
  prepname = paste("Prepared", "Weather", state, sep="_")
  ...
  if(length(grep(prepname, dir(file.path(localdir, "Weather")))) == 0) { 
    ...
    cat("Preparing", "Weather", state, "\n")
    
    # Read in GHCN data
    wx.files <- dir(file.path(localdir, "Weather"))
    wx.files <- wx.files[grep(state, wx.files)]
    wx.files <- wx.files[grep('csv$', wx.files)]
    wx <- vector()
    ...
    station_file <- file.path(localdir, "Weather", 'ghcnd-stations.txt')
    ...
    # Read in grid
    grid_shp <- readOGR(file.path(localdir, "Hex", state), 
                               layer = paste0(state, '_hexagons_1mi_neighbors'))
    grid_shp <- spTransform(grid_shp, CRS(proj.USGS))
    
    # Read in buffered state shapefile
    state_buff <- readOGR(censusdir, 
                          layer = paste0(state, "_buffered"))
    state_buff <- spTransform(state_buff, CRS(proj.USGS))
    
    # Clip grid to county shapefile
    grid_intersects <- gIntersects(state_buff, grid_shp, byid = T)
    ...
    writeLines(c(""), paste0("Prep_Weather_", state, "_log.txt"))    
    ...
    wx.grd.day <- foreach(day = unique(wx$DATE), 
      ...
      cat(paste(Sys.time()), as.character(day), "\n", 
          file = paste0("Prep_Weather_", state, "_log.txt"), append = T) 
          
      # Scan team bucket for completed daily weather prep ----
      fn = paste("Prep_Weather_Daily_", day,"_", state, ".RData", sep="")
