...
while (j <= length(wazefiles)){
  ...
  ll <- d %>%
         group_by(uuid) %>%
         summarise(reliability = max(reliability),
                   nrecord = n(),
                   date = max
  ...
  list_match <- aggregate(cbind(reliability, date) ~ uuid, data = bydate, FUN = max)
  ...
  list_match <- cbind(list_match, aggregate(date ~ uuid, data = bydate, FUN = max))
  ...
  #write to a csv
  aggregated_by_uuid1 <- merge(bydate, list_match, by = c("uuid","reliability"))
  aggregated_by_uuid2 <- aggregated_by_uuid1[!duplicated(aggregated_by_uuid1$uuid),]
  write.csv(aggregated_by_uuid2, file = paste("Z:/SDI Pilot Projects/Waze/MASTER Data Files/Waze Aggregated/", wazefiles[j], sep = ""))
  j = j+1
  }


    #redo with months in a clunky way
    setwd("Z:/SDI Pilot Projects/Waze/MASTER Data Files/Waze Aggregated")
