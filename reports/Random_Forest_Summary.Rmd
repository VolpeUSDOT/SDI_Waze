...
rownames(tabl) = c("Model 01 - 1 mile", "Model 04 - 4 mile", "Model 06 - 0.5 mile")
...
colnames(tabl)[1:5] = c("Accuracy", "Precision", "Recall", "False Positive Rate", "AUC")
rownames(tabl) = c("Model 03 - 1 mile", "Model 05 - 4 mile", "Model 07 - 0.5 mile")
# kable(tabl, 
...
colnames(tabl)[1:5] = c("Accuracy", "Precision", "Recall", "False Positive Rate", "AUC")
rownames(tabl) = c("Model 01 - 1 mile", "Model 08 - 1 mile, Neighbors")
...
colnames(tabl)[1:5] = c("Accuracy", "Precision", "Recall", "False Positive Rate", "AUC")
rownames(tabl) = c("Model 03 - 1 mile", "Model 09 - 1 mile, Neighbors")
...
colnames(tabl)[1:5] = c("Accuracy", "Precision", "Recall", "False Positive Rate", "AUC")
rownames(tabl) = c("Model 08 - 1 mile, Neighbors", "Model 12 - Weather", "Model 14 - Weather, Roads", "Model 16 - Weather, Roads, Jobs")
#kable(tabl, caption = "Phase 1 model diagnostics for April 2017, Maryland, with supplemental weather, road functional class, and economic data.", rownames = "Model")
datatable(tabl, filter = 'top',
...
colnames(tabl)[1:5] = c("Accuracy", "Precision", "Recall", "False Positive Rate", "AUC")
rownames(tabl) = c("Model 09 - 1 mile, Neighbors", "Model 13 - Weather", "Model 15 - Weather, Roads", "Model 17 - Weather, Roads, Jobs")
# kable(tabl, caption = "Phase 1 model diagnostics for April + May, test on June, Maryland, with supplemental weather, road functional class, and economic data.", rownames = "Model")
datatable(tabl, filter = 'top',
...
load(file.path(localdir, "Output_to_39"))
```
...
```{r bestphaseIIA, eval=T}
modelno = "23"

s3load(object = file.path(paste(outputdir, "fill0", sep="_"), paste("Model", modelno, "RandomForest_Output.RData", sep= "_")), bucket = waze.bucket)

imp.tab <- data.frame(round(rf.out$importance[order(rf.out$importance, decreasing = T)[1:15],], 2))
colnames(imp.tab) = "Gini importance"

kable(imp.tab, row.names = T,
      caption = "Variance importance for the highest recall plot in phase II, Model 23. Top 15 of 58 predictors shown") %>% 
  kableExtra::kable_styling(bootstrap_options = "striped")

varImpPlot(rf.out, main = "Model 23 - April-Sept, highest recall")
...
```{r phase2B}
# pairs to compare: 24-30
tabl <- vector()
mods = as.character(24:30)
for(i in mods){ tabl <- rbind(tabl, c(100*keyoutputs2[[i]]$diag, round(keyoutputs2[[i]]$auc, 4)))}
...
datatable(tabl, filter = 'top',
          ...
          formatStyle(2, background = styleEqual(max(tabl[,2]), 'lightgreen'))%>%
          formatStyle(3, background = styleEqual(max(tabl[,3]), 'lightgreen'))%>%
          formatStyle(4, background = styleEqual(max(tabl[,4]), 'lightgreen'))%>%
          formatStyle(5, background = styleEqual(min(tabl[,5]), 'lightgreen'))%>%
          formatStyle(7, background = styleEqual(max(tabl[,7]), 'lightgreen'))%>%
          formatStyle(8, background = styleEqual(min(tabl[,8]), 'lightgreen'))
```
...
tabl <- vector()
for(i in 1:length(keyoutputs2)){
  tabl <- rbind(tabl, c(Model = names(keyoutputs2)[i], 
                                    100*keyoutputs2[[i]][[3]],
