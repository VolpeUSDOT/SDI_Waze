...
colnames(tabl)[1:5] = c("Accuracy", "Precision", "Recall", "False Positive Rate", "AUC")
rownames(tabl) = c("03 - 1 mile", "05 - 4 mile", "07 - 0.5 mile")
...
load(file.path(localdir, "Output_to_32"))
```
...
Approximately 1.5% of the data includes times and locations where only EDT values, but no Waze accidents, were present. These account for 30,875 of the 2,003,391 observations for the April-September data. Omitting these observations had a negligable effect on the overall model performace, with minor increase in recall and decrease in precision, leading to an overall minor decrease in AUC. 

An even smaller quanity of data included times and locations where only Waze road closure events were reported, 2,892 of the total observations (0.1%). Omitting these interestingly lead to the highest recall of this set of model, but the best balance between recall and precision was still acheived with Model 30, including those observations.
...
datatable(tabl, filter = 'top',
          ...
          formatStyle(7, background = styleEqual(range(tabl[,7]), c('khaki','lightgreen')))%>%
          formatStyle(8, background = styleEqual(range(tabl[,8]), c('lightgreen','khaki')))
