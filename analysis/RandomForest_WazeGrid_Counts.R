...
#Remove Grid/times with no Waze user-reported events
dim(wazeTime.edt.hex)
wazeTime.edt.hex$hextime <- as.POSIXct(wazeTime.edt.hex$hextime)
wazeGridTime <- filter(wazeTime.edt.hex, nWazeAccident>0 | nWazeJam>0 | nWazeWeatherOrHazard>0)
w.04 <- wazeGridTime; rm(wazeGridTime)
dim(w.04)
...
write.csv(out.04,
          file = "RandomForest_pred_04_.csv",
          row.names = F)
...
#All Waze matches
omits = c(grep("GRID_ID", names(wGT.04), value = T), "day", "hextime", "year",
          "nMatchWaze_buffer", "nNoMatchWaze_buffer", 
          "nWazeAccident","nWazeJam", "nWazeRoadClosed", "nWazeWeatherOrHazard",
          "nWazeHazardCarStoppedRoad", "nWazeHazardCarStoppedShoulder",
          "nWazeHazardConstruction", "nWazeHazardObjectOnRoad", "nWazeHazardPotholeOnRoad", "nWazeHazardRoadKillOnRoad",
          "nWazeWeatherFlood", "nWazeWeatherFog", "nWazeHazardIceRoad",
          grep("EDT", names(wGT.04), value = T),
          "wx",
          "nWazeRowsInMatch", "uniqWazeEvents"
          #grep("nWazeAcc_", names(wGT.04), value = T), # neighboring accidents
          #grep("nWazeJam_", names(wGT.04), value = T) # neighboring jams
)

fitvars <- names(wGT.04)[is.na(match(names(wGT.04), omits))]
...
trainrows <- sort(sample(1:nrow(wGT.04), size = nrow(wGT.04)*.7, replace = F))
testrows <- (1:nrow(wGT.04))[!1:nrow(wGT.04) %in% trainrows]
...
system.time(rf.04 <- foreach(ntree = rep(ntree.use/avail.cores, avail.cores),
                             ...
                             .packages = "randomForest") %dopar%
              randomForest(wazeformula,
                           data = wGT.04[trainrows,],
                           ntree = ntree)
)


system.time(rf.04.pred <- predict(rf.04, wGT.04[testrows, fitvars]))

Nobs <- data.frame(t(c(nrow(wGT.04),
                       summary(wGT.04$uniqEDTreports),
                       length(wGT.04$nWazeAccident[wGT.04$nWazeAccident>0]) 
)))

colnames(Nobs) = c("N", "No EDT", "EDT present", "Waze accident present")
format(Nobs, big.mark = ",")

(predtab <- table(wGT.04$MatchEDT_buffer[testrows], rf.04.pred)) 
bin.mod.diagnostics(predtab)

# save output predictions

out.04 <- data.frame(w.04[testrows, c("GRID_ID", "day", "hour", "uniqEDTreports")], rf.04.pred)
out.04$day <- as.numeric(out.04$day)
names(out.04)[4:5] <- c("Obs", "Pred")
