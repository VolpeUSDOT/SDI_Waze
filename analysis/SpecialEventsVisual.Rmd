...
plotDateHeatMap <- function(dt, loc, weekday, metrics){
        ...
        dt_Date <- dt %>% group_by(Location.ID, DayofWeek, EventType, hour) %>% 
            summarize(numDay = length(unique(date)),
                      nWazeJam = sum(nWazeJam)/numDay,
                      Obs = sum(Obs)/numDay,
                      nWazeAccident = sum(nWazeAccident)/numDay,
                      nHazardOnShoulder = sum(nHazardOnShoulder)/numDay,
                      nHazardOnRoad = sum(nHazardOnRoad)/numDay,
                      nHazardWeather = sum(nHazardWeather)/numDay,
                      nEDTFatal = sum(nEDTFatal)/numDay,
                      nEDTCrashWPedBikeInv = sum(nEDTCrashWPedBikeInv)/numDay,
                      nEDTCrashWSchoolBusInv = sum(nEDTCrashWSchoolBusInv)/numDay,
                      nEDTVehCount = sum(nEDTVehCount)/numDay
                      )
         ...
         p + geom_tile() + # background colours are mapped according to the value column
            scale_fill_gradient2(low = muted("darkred"), 
                       mid = "white", 
                       high = muted("midnightblue"), 
                       midpoint = 0) + # determine the colour
            ggtitle(paste(location,",",buf, "Buffer", weekday)) + xlab("Hour") + ylab("Event Type") +
...
weekday = "Sunday"
metrics = "EDT Accident"
plotDateHeatMap(dt, loc, weekday, metrics) # average values
