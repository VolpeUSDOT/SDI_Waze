...
ggplot(dt_visual, aes(x = hour, y = nWazeJam, color = EventType, group = date)) + 
  ...
  geom_line(alpha = 0.5) +
  geom_rect(data = dt_visual,aes(xmin = ave(dt_visual$StartTime/100, dt_visual$date, FUN = function(x) mean(x)),xmax = ave(dt_visual$EndTime/100, dt_visual$date, FUN = function(x) mean(x)), ymin = -Inf,ymax=Inf), fill = dt_visual$background_col, alpha= 0.2/100, inherit.aes = FALSE) +
  facet_wrap(~ Month) +
...
ggplot(dt_visual, aes(x = hour, y = nWazeJam, color = EventType, group = date)) + 
  # geom_point(alpha = 0.5, color = "red") +
  geom_line(alpha = 0.5) +
  geom_rect(data = dt_visual,aes(xmin = ave(dt_visual$StartTime/100, dt_visual$date, FUN = function(x) unique(x)),xmax = ave(dt_visual$EndTime/100, dt_visual$date, FUN = function(x) mean(x)), ymin = -Inf,ymax=Inf), fill = dt_visual$background_col, alpha= 0.2/24, inherit.aes = FALSE) +
  facet_wrap(~ Month) +
  ylab("Average Waze Jam") +
  xlab("Hour of Day") +
  scale_color_manual(name = NULL, values=c("NoEvent" = "#999999", "Football" = "#E69F00", "Concert" = "#56B4E9")) +
