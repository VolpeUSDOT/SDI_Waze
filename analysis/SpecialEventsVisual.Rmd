...
* Number of Waze Jam
* Number of Hazard On Shoulder
* Number of Hazard On Road
* Number of Hazard Weather (nHazardWeather) - is it # of hazard or weather?

# Probability of NonCrash and Crash - real prob from RF model.
The probability has converted to based on the threshold ~ 0.2 using the built-in function in RF.

Following visualizations are for location "SE1" and 3 mile buffer zone.

To do:
- total number of accidents/jams
- number of accidents per gridcell per hour
- add shadebox for event duration
- boxplot of accidents over 42 grid cells.
- look at hour aggregation
- heatmap for each hour
- decrease buffer zone, 0.25, 0.5, 1 mile radius
