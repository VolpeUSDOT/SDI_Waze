...
# Adapted for SDC use
# Also written to create hex files for each state, based of national hexagon layer and the newly-created buffered files, for faster overlay in Hex_UA_Overlay_SDC.R
# However, Cstack limit reached: Hex subsetting must run outside of RStudio. In terminal run `ulimit -s unlimited`, then `Rscript datacleaning/Buffer_state`
...
# inherit states from ReduceWaze_SDC
