...
if(length(grep(prepname, dir(file.path(localdir, "Data","Crash")))) == 0) {
  
  cat("Preparing", "TN_Crash", g, "\n")
  
  load(file.path(localdir,'Data','Crash', 'TN_Crash_Simple_2008-2018.RData'))
  ...
  save(list = c("crash"),
       file = file.path(localdir,"Data","Crash", paste0(prepname, ".RData")))

  # Copy to local directory
  system(paste("cp",
               file.path(localdir,"Data","Crash", paste0(prepname, ".RData")),
               file.path(teambucket, "Data", "Crash", paste0(prepname, ".RData"))))

} else {
  load(file.path(localdir,"Data", "Crash", paste0(prepname, ".RData")))
}
